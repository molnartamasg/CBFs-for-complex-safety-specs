%% Simulation of reach-avoid problem on road network
clear; clc;

%% Problem setup
% Setup and parameters
% desired controller
par.xgoal=[4;8];	% goal point, m
...
% CBF contours
contour(xx1,xx2,hhc,[0,0],'Color',black,'Linewidth',2,'HandleVisibility','off');
contour(xx1,xx2,hh,[0,0],'Color',darkred,'Linewidth',2,'HandleVisibility','off');
% trajectory
...
%% Save results
resultname = ['road_kappa_',num2str(par.kappa)];
...
function h = hlogic(hi,type)
    ...
    % combine CBFs for each road
    Nr = length(hi)/2;
    hr = nan(Nr,1);
    for kr=1:Nr
        hr(kr) = h_and(hi(kr*2-1:kr*2));
    end
    % combine roads
    h = h_or(hr);
end
function [h,gradh] = gradhlogic(hi,gradhi,type)
    ...
    if strcmp(type,'smooth')
        ...
        gradhr = nan(Nr,2);
        for kr=1:Nr
            [~,gradhr(kr,:)] = gradrsum(hi(kr*2-1:kr*2),gradhi(kr*2-1:kr*2,:));
        end
        % combine roads
        gradh = sum(gradhr);
    else
